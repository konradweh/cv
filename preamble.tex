...
\newlength{\CVEntryGap}
\setlength{\CVEntryGap}{6pt} % constant space AFTER each entry
...
\newcommand{\cventry}[3]{%
  \begingroup
    % Make entry spacing deterministic (ignore global parskip/parindent)
    \setlength{\parskip}{0pt}%
    \setlength{\parindent}{0pt}%
...
% Optional helper for "clean" line breaks inside #2 (no extra vertical spacing)
% Use inside cventry bodies instead of \\ if you want deterministic spacing.
\newcommand{\cvline}{\newline}
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  % Photo block
  \coordinate (PhotoNW) at ([xshift=165mm,yshift=-\PhotoTopOffset]current page.north west);
  ...
  \draw[cvblue, line width=0.8pt]
    ...
    ([xshift=10mm+\SepLineX, yshift=28mm]current page.south west);

  % Horizontal line aligned to text width, 25mm above bottom
  \draw[cvblue, line width=0.8pt]
    ([yshift=18mm]current page text area.south west) --
    ([yshift=18mm]current page text area.south east);
  ...
  \smash{%
    \begin{tikzpicture}[remember picture,overlay]
      \node[anchor=north west, align=left] at
        ([xshift=10mm,yshift=-11mm]current page.north west)
      {%
